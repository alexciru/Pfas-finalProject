...
def pointclouds_from_masks(
    disparity_frame_: np.ndarray,
    img_: np.ndarray,
    obj_masks_: List[np.ndarray],
    Q: np.ndarray,
    min_pcd_size_: int,
) -> List[o3d.geometry.PointCloud]:
    # Get list of objects from both frames
    _objs_pointclouds = make_pointclouds_from_masks(
        disparity_frame_, img_, obj_masks_, Q
    )
...
def New_ObtainListOfPontClouds(
    disparity_frame_, n_frame_, left_img_, bb_boxes, Q, extr_mat
):
    # Get list of objects from both frames
    bb1 = bb_boxes[bb_boxes["frame"] == n_frame_]
    cluster_list1 = make_pointclouds_from_masks(
        disparity_frame_, left_img_, bb1, Q, extr_mat
    )
...
def make_pointclouds_from_masks(
    disparity_map, img_, obj_masks_: List[np.ndarray], Q
) -> List[o3d.geometry.PointCloud]:
    """
    ...
    for _i, _mask in enumerate(obj_masks_):
        ...
        # get point cloud
        _mask_points, _mask_points_colors = generate_pointCloud(
            maskedDisparity, img_, Q
        )
        ...
        _mask_pointcloud.colors = o3d.utility.Vector3dVector(
            _mask_points_colors / 255.0
        )
