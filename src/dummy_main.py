...
from ResultSaving import save_timeframe_results
...
def main():
    ...
    for n_frame in tqdm(range(2)):
        ...
        ex_mat = np.array(
            [
                [9.999838e-01, -5.012736e-03, -2.710741e-03, 5.989688e-02],
        ...
        clusterlist1 = make_pointclouds_from_masks(
            disparity_frame1, n_frame, left_img1, bb_boxes, Q, ex_mat
        )

        # TODO: no need to compute the clusters again if was computer in the previous frame
        clusterlist2 = make_pointclouds_from_masks(
            disparity_frame2, n_frame + 1, left_img2, bb_boxes, Q, ex_mat
        )
        ...
        save_timeframe_results(
            n_frame, None, clusterlist1, clusterlist2, filename="results.txt"
        )
